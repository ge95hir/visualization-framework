...
let moveValues = new Map();

// load SVG into HTML and execute initializing functions
$(document).ready(() => {
  let em = $('#embed');
  $.ajax({
    url: em.attr('load'),
...
function setMoveValues() {
  let elements = document.querySelectorAll("[visType*=history] rect[visValue=main]");
  elements.forEach((element) => {
    let id = element.getAttribute('id');
    moveValues.set(id, 0);
  })
...
function moveXYBy(visConcept, visValue) {
  ...
  allElements.forEach((element) => {
    ...
    let visOptions = helperGetVisOptions(element, defaultValue);
...
function historyFixed(visConcept) {
  ...
  groups.forEach((group) => {
    ...
    helperHistory(visConcept, group, quantity, distanceFactor, minValue, maxValue);
  })
};
...
// helper function, not its own operation
function helperHistory(visConcept, group, quantity, distanceFactor, minValue, maxValue) {
  element = group.querySelector("rect[visValue=main]");
  ...
  let rectID = element.getAttribute('id');
  let lastMoveValue = moveValues.get(rectID);
  ...
  if(minValue < 0) {
    ...
    if(transform != null) {
      ...
      matNew.d = matOld.d;
      matNew.e = matOld.e;
      matNew.f = matOld.f + lastMoveValue - moveValue;

      svgTransform.setMatrix(matNew);
    } else {
      xAxis.setAttribute('transform', `translate(0, ${-moveValue})`);
    }
    moveValues.set(rectID, moveValue);
  }
...
function helperGetVisOptions(element, defaultValue) {
  if(element.hasAttribute('visOptions')) {
    let result = [];
    let visOptions = element.getAttribute('visOptions').split(",");
    visOptions.forEach((item, index) => {
      if(item.trim() === "") {
        result.push(defaultValue[index]);
      }
      else {
        result.push(item);
      }
